...
func (suite *TransferTestSuite) TestHandleMsgTransfer() {
	...
	coins := sdk.NewCoins(
		...
		sdk.NewCoin("atuc", sdkmath.NewInt(10000000000)),
	)
	...
	coinswapKeeper.SetStandardDenom(suite.chainB.GetContext(), "atuc")
	...
	balanceCantoBefore := suite.chainB.App.(*app.Canto).BankKeeper.GetBalance(suite.chainB.GetContext(), suite.chainB.SenderAccount.GetAddress(), "atuc")
	...
	balanceCanto := suite.chainB.App.(*app.Canto).BankKeeper.GetBalance(suite.chainB.GetContext(), suite.chainB.SenderAccount.GetAddress(), "atuc")
