...
func (suite *IBCTestingSuite) SetupTest() {
	...
	// Mint coins on the gravity side which we'll use to unlock our atuc
	...
	// Mint coins on the cosmos side which we'll use to unlock our atuc
...
// CreatePool creates a pool with atuc and the given denom
func (suite *IBCTestingSuite) CreatePool(denom string) {
	coincanto := sdk.NewCoin("atuc", sdkmath.NewIntWithDecimal(10000, 18))
	...
	coinswapKeeper.SetStandardDenom(suite.cantoChain.GetContext(), "atuc")
