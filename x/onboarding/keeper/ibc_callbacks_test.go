...
func (suite *KeeperTestSuite) TestOnRecvPacket() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			false,
			sdk.NewCoins(sdk.NewCoin("atuc", sdkmath.ZeroInt())),
			sdk.NewCoin("atuc", sdkmath.ZeroInt()),
		...
		{
			"no swap / convert all transferred amount - no liquidity pool exists but atuc balance is already bigger than threshold",
			func() {
			...
			true,
			sdk.NewCoins(sdk.NewCoin("atuc", sdkmath.NewIntWithDecimal(4, 18))),
			sdk.NewCoin("atuc", sdkmath.NewIntWithDecimal(4, 18)),
			sdk.NewCoin(uusdtIbcdenom, sdkmath.ZeroInt()),
			transferAmount,
		},
		{
		...
		{
			"no swap / convert all transferred amount - atuc balance is already bigger than threshold",
			func() {
				transferAmount = sdkmath.NewIntWithDecimal(25, 6)
				transfer := transfertypes.NewFungibleTokenPacketData(denom, transferAmount.String(), secpAddrCosmos, ethsecpAddrcanto, "")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
		...
		{
			"swap / convert remaining ibc token - swap and erc20 conversion are successful (atuc balance is positive but less than threshold)",
			func() {
				transferAmount = sdkmath.NewIntWithDecimal(25, 6)
				transfer := transfertypes.NewFungibleTokenPacketData(denom, transferAmount.String(), secpAddrCosmos, ethsecpAddrcanto, "")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
			...
			true,
			sdk.NewCoins(sdk.NewCoin("atuc", sdkmath.NewIntWithDecimal(3, 18))),
			sdk.NewCoin("atuc", sdkmath.NewIntWithDecimal(7, 18)),
			sdk.NewCoin(uusdcIbcdenom, sdkmath.ZeroInt()),
			sdkmath.NewInt(20998399),
		},
		{
			...
			true,
			sdk.NewCoins(sdk.NewCoin("atuc", sdkmath.ZeroInt()), sdk.NewCoin(uusdcIbcdenom, sdkmath.NewIntWithDecimal(1, 6))),
			sdk.NewCoin("atuc", sdkmath.NewIntWithDecimal(4, 18)),
			sdk.NewCoin(uusdcIbcdenom, sdkmath.NewIntWithDecimal(1, 6)),
			sdkmath.NewInt(20998399),
		},
		{
			"swap / convert remaining ibc token - swap and erc20 conversion are successful (atuc and ibc token balance is bigger than 0)",
			func() {
				transferAmount = sdkmath.NewIntWithDecimal(25, 6)
				transfer := transfertypes.NewFungibleTokenPacketData(denom, transferAmount.String(), secpAddrCosmos, ethsecpAddrcanto, "")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
			...
			true,
			sdk.NewCoins(sdk.NewCoin("atuc", sdkmath.NewIntWithDecimal(3, 18)), sdk.NewCoin(uusdcIbcdenom, sdkmath.NewIntWithDecimal(1, 6))),
			sdk.NewCoin("atuc", sdkmath.NewIntWithDecimal(7, 18)),
			sdk.NewCoin(uusdcIbcdenom, sdkmath.NewIntWithDecimal(1, 6)),
			sdkmath.NewInt(20998399),
		},
		{
			...
			true,
			sdk.NewCoins(sdk.NewCoin("atuc", sdkmath.NewIntWithDecimal(3, 18))),
			sdk.NewCoin("atuc", sdkmath.NewIntWithDecimal(3, 18)),
			sdk.NewCoin(uusdcIbcdenom, sdkmath.ZeroInt()),
			transferAmount,
		},
		{
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			// Setup liquidity pool (atuc/uUSDC)
			suite.app.CoinswapKeeper.SetStandardDenom(suite.ctx, "atuc")
			coinswapParams := suite.app.CoinswapKeeper.GetParams(suite.ctx)
			coinswapParams.MaxSwapAmount = sdk.NewCoins(sdk.NewCoin(uusdcIbcdenom, sdkmath.NewIntWithDecimal(10, 6)))
			suite.app.CoinswapKeeper.SetParams(suite.ctx, coinswapParams)

			testutil.FundAccount(suite.app.BankKeeper, suite.ctx, secpAddr, sdk.NewCoins(sdk.NewCoin("atuc", sdkmath.NewIntWithDecimal(10000, 18)), sdk.NewCoin(uusdcIbcdenom, sdkmath.NewIntWithDecimal(10000, 6))))
			...
			// Check balances
			cantoBalance := suite.app.BankKeeper.GetBalance(suite.ctx, ethsecpAddr, "atuc")
