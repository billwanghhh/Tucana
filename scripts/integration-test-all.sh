...
## default port prefixes for tucd
NODE_P2P_PORT="2660"
NODE_PORT="2663"
NODE_RPC_PORT="2666"
...
init_func() {
    "$PWD"/build/tucd keys add $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --no-backup --algo "eth_secp256k1"
    "$PWD"/build/tucd init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
    "$PWD"/build/tucd add-genesis-account \
    "$("$PWD"/build/tucd keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000acanto,1000000000000000000stake \
    --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/tucd gentx "$KEY$i" 1000000000000000000stake --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/tucd collect-gentxs --home "$DATA_DIR$i"
    "$PWD"/build/tucd validate-genesis --home "$DATA_DIR$i"
...
start_func() {
    ...
    "$PWD"/build/tucd start --pruning=nothing --rpc.unsafe \
