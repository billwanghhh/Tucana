...
build-docker:
	...
	mkdir -p ./build/
	$(DOCKER) cp canto:/usr/bin/tucd ./build/
...
ifeq ($(OS),Windows_NT)
	...
	IF not exist "build/node0/$(canto_BINARY)/config/genesis.json" docker run --rm -v $(CURDIR)/build\canto\Z tucd/node "./tucd testnet --v 4 -o /canto --keyring-backend=test --ip-addresses tucdnode0,tucdnode1,tucdnode2,tucdnode3"
	docker-compose up -d
else
	mkdir -p localnet-setup
	@$(MAKE) localnet-build

	if ! [ -f localnet-setup/node0/$(canto_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/localnet-setup:/canto:Z tucd/node "./tucd testnet --v 4 -o /canto --keyring-backend=test --ip-addresses tucdnode0,tucdnode1,tucdnode2,tucdnode3"; fi
	docker-compose up -d
endif
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\localnet-setup\node0\tucd:canto\Z tucd/node "./tucd unsafe-reset-all --home=/canto"
	@docker run --rm -v $(CURDIR)\localnet-setup\node1\tucd:canto\Z tucd/node "./tucd unsafe-reset-all --home=/canto"
	@docker run --rm -v $(CURDIR)\localnet-setup\node2\tucd:canto\Z tucd/node "./tucd unsafe-reset-all --home=/canto"
	@docker run --rm -v $(CURDIR)\localnet-setup\node3\tucd:canto\Z tucd/node "./tucd unsafe-reset-all --home=/canto"
else
	@docker run --rm -v $(CURDIR)/localnet-setup/node0/tucd:/canto:Z tucd/node "./tucd unsafe-reset-all --home=/canto"
	@docker run --rm -v $(CURDIR)/localnet-setup/node1/tucd:/canto:Z tucd/node "./tucd unsafe-reset-all --home=/canto"
	@docker run --rm -v $(CURDIR)/localnet-setup/node2/tucd:/canto:Z tucd/node "./tucd unsafe-reset-all --home=/canto"
	@docker run --rm -v $(CURDIR)/localnet-setup/node3/tucd:/canto:Z tucd/node "./tucd unsafe-reset-all --home=/canto"
endif
