...
async function main() {
  ...
  const { address: chainAddress, contract: chain } = await deployContract("ChainContract", Chain, configAddress);
  ...
  const { address: collateral_1 } = await deployContract("MockToken", MockToken);
  const { address: collateral_2 } = await deployContract("MockToken", MockToken);
  const { address: reward_token } = await deployContract("MockToken", MockToken);
